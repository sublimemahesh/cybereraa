$(function () {
    ...
    function createPackage(e) {
        ...
        axios.post(`${APP_URL}/admin/staking-packages`, formData)
            .then(response => {
                Toast.fire({
            ...
            let errorMap = ['name', 'amount', 'gas_fee', 'description', 'is_active']
            errorMap.map(id => {
    ...
    function updatePackage(e) {
        ...
        $('#package-form').find(".text-danger").remove();
        let formData = $('#package-form').serialize();
        axios.patch(`${APP_URL}/admin/staking-packages/${PACKAGE_ID}`, formData)
            .then(response => {
                Toast.fire({
                    icon: response.data.icon, title: response.data.message,
                }).then(res => {
    ...
    $(document).on("click", ".delete-package", function (e) {
        ...
        Swal.fire({
            ...
            if (willDelete.isConfirmed) {
                loader()
                axios.delete(`${APP_URL}/admin/staking-packages/${package_id}`)
                    .then(response => {
                        Toast.fire({
                            icon: response.data.icon, title: response.data.message,
                        }).then(res => {
